...
def ctrlc_handler(sig_num, frm):
    with open("./countries.json", "w+", encoding='utf-8') as new_f:
...
with open("./countries.json", 'r', encoding='utf-8') as f:
    ...
    countries = src_data['countries']
    count = 0
    try:
        for country in countries:
            count += 1
            if 'zh_name' not in dict(country).keys():
                # if 'zh_name' not in city.keys():
                if country['name'] != '':
                    zh_name = translate(country['name'])
                    zh_name = zh_name.replace("酒店", "")
                    country['zh_name'] = zh_name
                    print("[%d/%d] [%s]==>[%s]" %
                          (count, len(countries), country['name'], zh_name))
                    sleep(1)
                else:
                    country['zh_name'] = country['name']
            else:
                country['zh_name'] = str(country['zh_name']).replace("酒店", "")
                print("[%d/%d] [%s]==>[%s]" %
                      (count, len(countries), country['name'],
                       country['zh_name']))
    except Exception as e:
        print(e)
    finally:
