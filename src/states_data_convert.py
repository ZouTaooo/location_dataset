...
def ctrlc_handler(sig_num, frm):
    with open("./states.json", "w+", encoding='utf-8') as new_f:
...
with open("./states.json", 'r', encoding='utf-8') as f:
    ...
    states = src_data['states']
    count = 0
    try:
        for state in states:
            count += 1
            if 'zh_name' not in dict(state).keys():
                # if 'zh_name' not in city.keys():
                if state['name'] != '':
                    zh_name = translate(state['name'])
                    state['zh_name'] = zh_name
                    print("[%d/%d] [%s]==>[%s]" %
                          (count, len(states), state['name'], zh_name))
                    sleep(1)
                else:
                    state['zh_name'] = state['name']
            else:
                state['zh_name'] = str(state['zh_name']).replace("酒店", "")
                print("[%d/%d] [%s]==>[%s]" %
                      (count, len(states), state['name'], state['zh_name']))
    except Exception as e:
        print(e)
    finally:
        with open("./states.json", "w+", encoding='utf-8') as new_f:
